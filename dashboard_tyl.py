...
ASESORES = [
    ...
    "JENIFFER QUEVEDO MURILLO",
    "WILFREDDY SURINCHO CESPEDES",
    "LUZ CAMARGO IBAÑEZ",
]
...
SHORT = {
    ...
    "JENIFFER QUEVEDO MURILLO":        "Jennifer Q.",
    "WILFREDDY SURINCHO CESPEDES":     "Wilfreddy S.",
    "LUZ CAMARGO IBAÑEZ":              "Luz C.",
}
SHORT = {norm(k): v for k, v in SHORT.items()}

# ── ESTADO DEL EQUIPO ─────────────────────────────────────────────────────
# "activo": sigue en el equipo comercial hoy.
# "inactivo": ya no está en la empresa (renuncia), pero sus ventas
#   históricas -- y las que se alcanzaron a cargar del mes en que salió --
#   se mantienen en el reporte (no se borran ni se ocultan). Simplemente no
#   se le asigna meta hacia adelante ni se le exige cumplimiento.
ESTADO_ASESORES = {
    "DIANA ROSSMARY RAMIREZ RIAÑO":     "activo",
    "INGRID YURLEY CHURIO PATIÑO":      "activo",
    "JEISSON IVAN CHITIVA VALLEJO":     "activo",
    "LUIS GABRIEL ACOSTA CIRO":         "activo",
    "NÉSTOR ARNOLDO USECHE TRIANA":     "inactivo",   # renunció
    "ADRIANA CAROLINA CUEVAS MONSALVE": "inactivo",   # renunció
    "JENIFFER QUEVEDO MURILLO":         "activo",
    "WILFREDDY SURINCHO CESPEDES":      "activo",     # reingresó (ya había estado en años anteriores)
    "LUZ CAMARGO IBAÑEZ":               "activo",     # se vincula en septiembre 2026
}
ESTADO_ASESORES = {norm(k): v for k, v in ESTADO_ASESORES.items()}

# Asesores SIN meta fija asignada por diseño (no es un dato que falte
# cargar): sus ventas SÍ suman en los totales de facturación del equipo,
# pero no tienen meta ni entran en el % de cumplimiento ni en las alertas
# de "facturación baja/crítica".
SIN_META_FIJA = {norm("JENIFFER QUEVEDO MURILLO")}

def etiqueta_asesor(a):
    """Nombre corto + una marca visible cuando el asesor ya no está activo
    o no tiene meta fija -- para que quede claro en tablas y selectores sin
    tener que adivinar por qué su % de meta se ve distinto."""
    base = SHORT.get(a, a)
    if a in SIN_META_FIJA:
        return f"{base} (sin meta fija)"
    if ESTADO_ASESORES.get(a) == "inactivo":
        return f"{base} (inactivo)"
    return base
...
# de metas correspondiente. Un mes puede quedar PARCIAL (con la meta de
# solo algunos asesores) mientras se consiguen los datos de los demás -- ver
# `metas_para_mes` más abajo, que resuelve cada asesor por separado.
METAS_POR_MES = {
    # Histórico compartido por la gerencia para Jeisson, Diana, Gabriel e
    # Ingrid. Néstor y Adriana no eran asesores todavía en estos meses (se
    # vincularon después) -- por eso no tienen meta aquí; en esos meses
    # usan la meta definida más cercana que sí tengan (ver metas_para_mes).
    (2025, 9): {
        "JEISSON IVAN CHITIVA VALLEJO": 132_000_000,
        "DIANA ROSSMARY RAMIREZ RIAÑO": 132_000_000,
        "LUIS GABRIEL ACOSTA CIRO":     132_000_000,
        "INGRID YURLEY CHURIO PATIÑO":  100_000_000,
    },
    (2025, 10): {
        "JEISSON IVAN CHITIVA VALLEJO": 132_000_000,
        "DIANA ROSSMARY RAMIREZ RIAÑO": 132_000_000,
    ...
    (2026, 1): {  # Enero 2026
        "JEISSON IVAN CHITIVA VALLEJO": 66_000_000,
        "DIANA ROSSMARY RAMIREZ RIAÑO": 66_000_000,
        "LUIS GABRIEL ACOSTA CIRO":     66_000_000,
        "INGRID YURLEY CHURIO PATIÑO":  50_000_000,
    },
    (2026, 2): {
        "JEISSON IVAN CHITIVA VALLEJO": 150_000_000,
        "DIANA ROSSMARY RAMIREZ RIAÑO": 150_000_000,
        "LUIS GABRIEL ACOSTA CIRO":     150_000_000,
        "INGRID YURLEY CHURIO PATIÑO":  120_000_000,
    },
    (2026, 3): {
        "JEISSON IVAN CHITIVA VALLEJO": 150_000_000,
        "DIANA ROSSMARY RAMIREZ RIAÑO": 150_000_000,
    ...
    (2026, 6): {  # Junio 2026 -- meta completa (los 6 asesores)
        "DIANA ROSSMARY RAMIREZ RIAÑO":    150_000_000,
    ...
    (2026, 7): {  # Julio 2026 (parcial) -- meta completa (los 6 asesores)
        "DIANA ROSSMARY RAMIREZ RIAÑO":    150_000_000,
        "INGRID YURLEY CHURIO PATIÑO":     150_000_000,
        "JEISSON IVAN CHITIVA VALLEJO":    150_000_000,
        "LUIS GABRIEL ACOSTA CIRO":        150_000_000,
    ...
    (2026, 8): {  # Agosto 2026 -- metas de la imagen compartida por gerencia.
        # Néstor renunció: queda en 0 de forma explícita (sus ventas de
        # agosto que sí se alcanzaron a cargar se siguen sumando al total
        # del equipo, solo que sin meta que cumplir). Adriana también
        # renunció y no tiene ventas ni meta este mes -- al no tener meta
        # fija por diseño (ver SIN_META_FIJA), no necesita entrada aquí.
        "DIANA ROSSMARY RAMIREZ RIAÑO":    150_000_000,
        "INGRID YURLEY CHURIO PATIÑO":     150_000_000,
        "JEISSON IVAN CHITIVA VALLEJO":    150_000_000,
        "LUIS GABRIEL ACOSTA CIRO":        150_000_000,
        "NÉSTOR ARNOLDO USECHE TRIANA":              0,
        "WILFREDDY SURINCHO CESPEDES":     80_000_000,
    },
    (2026, 9): {  # Septiembre 2026 -- misma meta que agosto (confirmado con gerencia)
        # Néstor y Adriana: renunciaron, sin meta desde este mes (Néstor
        # queda en 0 explícito; Adriana no tiene ventas ni meta este mes y
        # ya no aparece aquí -- metas_para_mes la resuelve en 0 por su
        # estado "inactivo").
        "DIANA ROSSMARY RAMIREZ RIAÑO":    150_000_000,
        "INGRID YURLEY CHURIO PATIÑO":     150_000_000,
        "JEISSON IVAN CHITIVA VALLEJO":    150_000_000,
        ...
        "NÉSTOR ARNOLDO USECHE TRIANA":              0,
        "WILFREDDY SURINCHO CESPEDES":     80_000_000,
        "LUZ CAMARGO IBAÑEZ":              50_000_000,  # se vincula este mes, sin ventas todavia
    },
    (2026, 10): {  # Octubre 2026 -- misma meta que agosto (confirmado con gerencia)
        "DIANA ROSSMARY RAMIREZ RIAÑO":    150_000_000,
        "INGRID YURLEY CHURIO PATIÑO":     150_000_000,
        "JEISSON IVAN CHITIVA VALLEJO":    150_000_000,
        "LUIS GABRIEL ACOSTA CIRO":        150_000_000,
        "NÉSTOR ARNOLDO USECHE TRIANA":              0,
        "WILFREDDY SURINCHO CESPEDES":     80_000_000,
        "LUZ CAMARGO IBAÑEZ":              60_000_000,
    },
    (2026, 11): {  # Noviembre 2026 -- misma meta que agosto (confirmado con gerencia)
        "DIANA ROSSMARY RAMIREZ RIAÑO":    150_000_000,
        "INGRID YURLEY CHURIO PATIÑO":     150_000_000,
        "JEISSON IVAN CHITIVA VALLEJO":    150_000_000,
        "LUIS GABRIEL ACOSTA CIRO":        150_000_000,
        "NÉSTOR ARNOLDO USECHE TRIANA":              0,
        "WILFREDDY SURINCHO CESPEDES":     80_000_000,
        "LUZ CAMARGO IBAÑEZ":              70_000_000,
    },
    (2026, 12): {  # Diciembre 2026 (PARCIAL) -- por ahora solo se tiene
        # confirmada la meta de Luz. Al resto les falta la cifra oficial de
        # gerencia para este mes; mientras tanto usan su meta más reciente
        # (ver metas_para_mes) y la UI avisa que este mes esta parcial.
        "LUZ CAMARGO IBAÑEZ":              40_000_000,
    },
}
...
def metas_para_mes(anio, mes):
    """Devuelve (metas, exacta): dos dicts asesor -> valor / asesor -> bool.
    Cada asesor se resuelve de forma INDEPENDIENTE: si (anio,mes) no trae su
    meta puntual todavía (p.ej. a Ingrid le falta un mes que a Diana no),
    usa la meta definida más cercana en el tiempo PARA ESE ASESOR (la
    anterior más próxima; si no hay ninguna anterior, la más antigua
    disponible) y lo marca exacta=False para que la UI pueda avisar
    puntualmente cuáles asesores están usando una meta de respaldo."""
    metas, exacta = {}, {}
    for a in ASESORES:
        valor_exacto = METAS_POR_MES.get((anio, mes), {}).get(a)
        if valor_exacto is not None:
            metas[a], exacta[a] = valor_exacto, True
            continue
        # Asesor inactivo (ya no está en la empresa) o sin meta fija por
        # diseño: si no hay una cifra puntual cargada para este mes, la
        # meta es 0 de forma EXACTA -- no es un dato pendiente de cargar
        # (no se debe avisar "falta meta oficial"), y no se le sigue
        # arrastrando para siempre la última meta que tuvo cuando estaba
        # activo.
        if a in SIN_META_FIJA or ESTADO_ASESORES.get(a) == "inactivo":
            metas[a], exacta[a] = 0, True
            continue
        disponibles = sorted(ym for ym, d in METAS_POR_MES.items() if a in d)
        if not disponibles:
            metas[a], exacta[a] = 0, False
            continue
        anteriores = [ym for ym in disponibles if ym <= (anio, mes)]
        ref = max(anteriores) if anteriores else min(disponibles)
        metas[a], exacta[a] = METAS_POR_MES[ref][a], False
    return metas, exacta
...
SEMANA_DIAS     = {}
TOTAL_DIAS_MES  = 30
COLORS = ["#378ADD","#1D9E75","#7F77DD","#D85A30","#BA7517","#D4537E","#2CA02C","#8C564B"]
COLOR_MAP = {a: COLORS[i % len(COLORS)] for i, a in enumerate(ASESORES)}
...
# Indicador de "¿quién entra a verlo y cuántas veces al día?": cada vez que
# alguien abre la app se le pide identificarse (eligiendo su nombre de una
# lista desplegable -- no texto libre, para evitar errores de tipeo) y se
# guarda un registro en un JSON local junto al script, una vez por sesión
# de navegador.
# HONESTIDAD SOBRE EL NIVEL DE CONFIABILIDAD: esto NO es una autenticación
# real -- cualquiera puede elegir el nombre de otra persona en la lista. Es
# un "honor system" pensado para que gerencia pueda ver, día a día, si el
# equipo comercial está entrando a revisar el tablero -- no para control de
# acceso ni seguridad. Si en algún momento se necesita una identificación
# verificada de verdad, la única forma confiable es restringir el link en
# Streamlit Cloud para que cada persona inicie sesión con su cuenta de
# Google antes de poder ver la app.
# LIMITACIÓN ADICIONAL: si el dashboard corre en Streamlit Community Cloud,
# este archivo vive en el contenedor de la app y puede reiniciarse si la
# app se "duerme" por inactividad prolongada o se redespliega desde cero.
LOG_FILE = os.path.join(FOLDER, "visor_log.json")

def registrar_visita(quien):
    """Registra una apertura del dashboard identificada por `quien` (nombre
    corto de asesor, 'Gerencia', 'Otro' o 'Sin identificar'). Usa
    session_state para contar una sola vez por sesión de navegador (evita
    inflar el contador con cada rerun que dispara un filtro)."""
    if st.session_state.get("_visita_registrada"):
        return st.session_state.get("_visita_log", {"total": 0, "registros": []})
    ...
    try:
        if os.path.exists(LOG_FILE):
            ...
            log = {"total": 0, "registros": []}
    except Exception:
        log = {"total": 0, "registros": []}
    log.setdefault("registros", [])
    ...
    log["registros"] = log["registros"][-3000:]  # cap para no crecer indefinido
    try:
...
def leer_log():
    """Lee el log de aperturas de disco sin registrar nada (solo lectura,
    para mostrar el acumulado real de todo el equipo en pantalla)."""
    try:
        with open(LOG_FILE, "r", encoding="utf-8") as f:
            log = json.load(f)
        log.setdefault("registros", [])
        log.setdefault("total", len(log["registros"]))
        return log
    except Exception:
        return {"total": 0, "registros": []}

def resumen_accesos_hoy(log):
    """Tabla: aperturas de HOY por cada persona de la lista (asesores +
    Gerencia/Otro/Sin identificar), para que gerencia vea de un vistazo
    quién ha entrado hoy y quién no."""
    hoy = _dt.date.today().isoformat()
    registros = [r for r in log.get("registros", []) if r.get("fecha") == hoy]
    personas = [SHORT.get(a, a) for a in ASESORES] + ["Líder Comercial", "Gerencia", "Externo 1", "Externo 2"]
    rows = []
    for p in personas:
        n = sum(1 for r in registros if r.get("quien") == p)
        rows.append({"Persona": p, "Aperturas hoy": n,
                     "Entró hoy": "✅" if n > 0 else "—"})
    return pd.DataFrame(rows)

def chart_accesos_por_dia(log, dias=14):
    """Barras apiladas: aperturas por día (últimos `dias` días) coloreadas
    por persona -- para ver de un vistazo la frecuencia de consulta."""
    registros = log.get("registros", [])
    hoy = _dt.date.today()
    fechas = [(hoy - _dt.timedelta(days=i)).isoformat() for i in range(dias - 1, -1, -1)]
    personas = [SHORT.get(a, a) for a in ASESORES] + ["Líder Comercial", "Gerencia", "Externo 1", "Externo 2"]
    colores = COLORS + ["#B0AEA6", "#6E6B63", "#2E2C28", "#8C8A83"]
    fig = go.Figure()
    for i, p in enumerate(personas):
        ys = [sum(1 for r in registros if r.get("fecha") == f and r.get("quien") == p)
              for f in fechas]
        if sum(ys) == 0:
            continue
        fig.add_bar(x=fechas, y=ys, name=p, marker_color=colores[i % len(colores)])
    fig.update_layout(barmode="stack", height=260, margin=dict(l=0,r=0,t=10,b=0),
                      yaxis=dict(title="Aperturas", gridcolor="#f0f0f0", dtick=1),
...
def load_data(files, _mod_times=None):
    ...
    # Fecha efectiva para ubicar la factura en semana/mes: se usa la fecha
    # real de la factura y, SOLO si esta viene vacía (típico de facturas
    # canceladas en Odoo), se usa la "Fecha de vencimiento" como respaldo --
    # así la factura sí queda ubicada en un mes/semana en vez de desaparecer
    # del reporte. Se guarda constancia de cuáles quedaron con fecha
    # estimada para poder avisarlo en pantalla.
    df_f["_fecha_estimada"] = (df_f["Fecha de la factura"].isna() &
                                df_f["Fecha de vencimiento"].notna())
    _fecha_periodo = df_f["Fecha de la factura"].fillna(df_f["Fecha de vencimiento"])
    df_f["_fecha_efectiva"] = _fecha_periodo
    # "Int64" (nullable) en vez de "int": si AMBAS fechas vienen vacías
    # (caso raro) con int estricto esto rompía toda la carga (ValueError al
    # convertir NaN a int). Con Int64 esa fila queda con semana/anio_mes en
    # blanco -- no rompe la app, y se avisa en pantalla cuántas son.
    df_f["semana"]   = _fecha_periodo.dt.isocalendar().week.astype("Int64")
    df_f["anio_mes"] = _fecha_periodo.dt.to_period("M")
    df_f["Total"]    = df_f["Importe sin impuestos en la moneda firmada"]

    _sin_ninguna_fecha = (df_f["Fecha de la factura"].isna() &
                          df_f["Fecha de vencimiento"].isna())
    ...
    estimadas_n     = int(df_f["_fecha_estimada"].sum())
    estimadas_valor = float(df_f.loc[df_f["_fecha_estimada"], "Total"].sum())
    sin_fecha_n     = int(_sin_ninguna_fecha.sum())
    sin_fecha_valor = float(df_f.loc[_sin_ninguna_fecha, "Total"].sum())

    return {"fact": df_f, "cart": df_c, "vis": df_v, "cot": df_cot,
            "estimadas_n": estimadas_n, "estimadas_valor": estimadas_valor,
...
def calc_kpis(filt, asesores_sel, semanas_sel):
    ...
    # Meta OFICIAL completa del mes (sin prorratear por semanas/días
    # transcurridos) -- consistente con calc_kpis_periodo (Trimestral/
    # Histórico). Antes se multiplicaba por frac_mes(semanas_sel), lo que
    # mostraba una meta reducida mientras el mes estuviera en curso (p.ej.
    # $125.8M en vez de los $150M oficiales de un asesor), dando un % de
    # cumplimiento que no correspondía a la meta real acordada con gerencia.
    tot_meta = sum(METAS.get(a, 0) for a in asesores_sel)
    pct_meta = (tot_fact / tot_meta * 100) if tot_meta else 0
    tot_pend = filt["cart"]["Importe pendiente firmado"].sum()
    tot_vis  = len(filt["vis"])
    avg_vis  = tot_vis / (len(asesores_sel) * n_sem) if asesores_sel and n_sem else 0
...
def tabla_visitas(vis_df, asesores_sel, semanas_sel):
    ...
    for a in asesores_sel:
        ...
        rows.append({"Asesor": etiqueta_asesor(a), "Visitas": tv,
...
def tabla_resumen_periodo(filt, asesores_sel, meses_incluidos):
    ...
    for a in asesores_sel:
        ...
        rows.append({
            "Asesor": etiqueta_asesor(a),
            "Facturación": fmt_m(ft),
            "Meta": fmt_m(meta),
            "% Meta": "N/A" if meta == 0 else f"{pct:.0f}%",
            "Visitas": vis,
...
def tabla_alertas(filt, asesores_sel, semanas_sel):
    ...
    for a in asesores_sel:
        ...
        tasa = gan / cer * 100 if cer else 0
        s    = etiqueta_asesor(a)
        # Solo se evalúa cumplimiento de facturación si el asesor tiene
        # meta > 0: con meta=0 (inactivo o sin meta fija) no aplica alerta
        # de "facturación crítica/baja" -- no es que le esté yendo mal, es
        # que no tiene una meta que cumplir.
        if meta > 0:
            if pct < 30:
...
def main():
    ...
    # Identificación de quién abre el tablero -- se pide ANTES de mostrar
    # cualquier dato, y el resto de la app NO se carga hasta que se elija
    # un nombre. No es una autenticación real, ver nota en registrar_visita().
    with st.sidebar:
        st.markdown("#### 👤 Identifícate")
        opciones_id = ["Selecciona tu nombre..."] + [SHORT[a] for a in ASESORES] + ["Líder Comercial", "Gerencia", "Externo 1", "Externo 2"]
        quien_sel = st.selectbox("¿Quién eres?", opciones_id, key="quien_soy",
                                 label_visibility="collapsed",
                                 help="Para que gerencia pueda ver cuántas veces al día "
                                      "entra cada asesor a revisar el tablero.")

    if quien_sel == "Selecciona tu nombre...":
        st.title("📊 Dashboard Comercial — TYL Herramientas y Seguridad SAS")
        st.info("👈 Para ver la información del tablero, primero selecciona tu "
                "nombre en la barra lateral ('¿Quién eres?').")
        st.stop()

    with st.sidebar:
        st.divider()

    # Se llega aquí solo si ya se identificó -- se registra la apertura
    # (una vez por sesión de navegador) y se sigue cargando el resto.
    registrar_visita(quien_sel)
    log_visitas = leer_log()
    ...
    if data.get("estimadas_n", 0) > 0:
        st.info(
            f"ℹ️ {data['estimadas_n']} factura(s) sin 'Fecha de la factura' "
            f"(total {fmt_m(data['estimadas_valor'])}) se ubicaron usando su "
            "'Fecha de vencimiento' como aproximación -- revisa en Odoo si el "
            "monto es significativo.")
    if data.get("sin_fecha_n", 0) > 0:
        st.warning(
            f"⚠️ {data['sin_fecha_n']} factura(s) no tienen NINGUNA fecha "
            f"('Fecha de la factura' ni 'Fecha de vencimiento') en el Excel "
            f"(total {fmt_m(data['sin_fecha_valor'])}). No se pueden ubicar en "
            "ningún mes/semana/trimestre, así que no aparecen en Mensual, "
            "Trimestral ni en la gráfica Histórica -- revisa esas facturas en "
            "Odoo si el monto es significativo.")
    ...
    with st.sidebar:
        ...
        sel_asesor = st.selectbox(
            ...
            format_func=lambda x: "Todos los asesores" if x=="Todos" else etiqueta_asesor(x))
        asesores_sel = ASESORES if sel_asesor == "Todos" else [sel_asesor]

        inactivos_lbl = [SHORT.get(a,a) for a in ASESORES if ESTADO_ASESORES.get(a) == "inactivo"]
        sin_meta_lbl  = [SHORT.get(a,a) for a in ASESORES if a in SIN_META_FIJA]
        if inactivos_lbl:
            st.caption(f"⚠️ Ya no están en el equipo: {', '.join(inactivos_lbl)}. "
                       "Sus ventas históricas (y las que alcanzaron a cargarse "
                       "después de su salida) se mantienen en el reporte, sin meta hacia adelante.")
        if sin_meta_lbl:
            st.caption(f"ℹ️ Sin meta fija asignada: {', '.join(sin_meta_lbl)}. "
                       "Sus ventas suman en positivo al total del equipo, pero no "
                       "tienen meta ni entran en el % de cumplimiento.")
        ...
        if vista == "Mensual":
            ...
            SEMANA_LABELS, SEMANA_DIAS, TOTAL_DIAS_MES = build_semana_info(
                pd.concat([data_mes["fact"]["_fecha_efectiva"], data_mes["vis"]["Iniciar"]])
            )
        ...
        # Indicador de uso del dashboard -- "¿quién entra y cuántas veces?"
        st.divider()
        st.subheader("👀 Uso del dashboard")
        registros = log_visitas.get("registros", [])
        hoy_iso = _dt.date.today().isoformat()
        vistas_hoy = sum(1 for r in registros if r.get("fecha") == hoy_iso)
        st.metric("Aperturas totales registradas", log_visitas.get("total", 0),
                  f"Hoy: {vistas_hoy}")

        st.caption("**Hoy, por persona:**")
        df_hoy = resumen_accesos_hoy(log_visitas)
        st.dataframe(df_hoy, use_container_width=True, hide_index=True)

        with st.expander("📈 Aperturas por día (últimos 14 días)"):
            st.plotly_chart(chart_accesos_por_dia(log_visitas),
                            use_container_width=True, config={"displayModeBar": False})

        with st.expander("Ver últimos accesos"):
            for r in reversed(registros[-15:]):
                st.caption(f"{r.get('fecha','')} {r.get('hora','')} — {r.get('quien','')}")

        st.caption("⚠️ Esto NO es una autenticación real: se basa en que cada "
                   "quien elija honestamente su nombre al entrar (nadie verifica "
                   "la identidad). Si el dashboard se redespliega o pasa mucho "
                   "tiempo sin uso, el contador puede reiniciarse.")
    ...
    if vista == "Mensual" and not all(metas_exactas.values()):
        asesores_respaldo = [SHORT.get(a, a) for a in ASESORES if not metas_exactas.get(a, False)]
        st.warning(f"⚠️ Para {periodo_label} aún no hay meta oficial cargada de: "
                   f"{', '.join(asesores_respaldo)}. Mientras tanto se usa la meta "
                   "definida más cercana en el tiempo para cada uno — pide que se "
                   "agreguen sus cifras oficiales de este mes en `METAS_POR_MES`.")
    ...
    if vista == "Mensual":
        ...
        k2.metric("Cumplimiento meta",
                  "Sin meta" if kpi["tot_meta"]==0 else f"{kpi['pct_meta']:.1f}%",
                  "No aplica (sin meta fija)" if kpi["tot_meta"]==0
                  else ("✓ Alcanzada" if kpi["pct_meta"]>=100
                  else ("En progreso" if kpi["pct_meta"]>=70 else "⚠ Por debajo")),
                  delta_color="off" if kpi["tot_meta"]==0
                  else ("normal" if kpi["pct_meta"]>=70 else "inverse"))
