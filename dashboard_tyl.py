...
def main():
    ...
    # Semanas reales presentes en los datos (facturación + visitas), no una
    # lista fija — así ninguna semana queda invisible en el filtro, y la
    # meta se prorratea por días reales del mes, no por nº de semanas.
    global SEMANA_LABELS, SEMANA_DIAS, TOTAL_DIAS_MES
    SEMANA_LABELS, SEMANA_DIAS, TOTAL_DIAS_MES = build_semana_info(
        pd.concat([data["fact"]["Fecha de la factura"], data["vis"]["Iniciar"]])
    )
