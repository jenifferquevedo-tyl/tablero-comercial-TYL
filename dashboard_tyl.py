...
def load_data(files, _mod_times=None):
    ...
    df_raw = df_raw[df_raw["_vend"].notna()].copy()

    # ── CORRECCIÓN: Excluir DMC que son par exacto de una factura Revertida ──
    # En Odoo, cuando se revierte una factura (FET→Revertido), se genera un DMC
    # con valor negativo idéntico. Ambos se cancelan mutuamente:
    #   - La FET Revertida ya queda fuera por VALID_STATES
    #   - El DMC también debe excluirse (de lo contrario descuenta dos veces)
    revertidas = df_raw[df_raw["Estado en pago"] == "Revertido"].copy()
    dmc_all    = df_raw[df_raw["Número"].str.startswith("DMC", na=False) &
                        df_raw["Estado en pago"].isin(VALID_STATES)].copy()
    dmc_excluir = set()
    for _, rev_row in revertidas.iterrows():
        v_abs = abs(rev_row["Importe sin impuestos en la moneda firmada"])
        vend  = rev_row["_vend"]
        matches = dmc_all[
            (dmc_all["_vend"] == vend) &
            (abs(dmc_all["Importe sin impuestos en la moneda firmada"]).round(2) == round(v_abs, 2))
        ]
        for dmc_num in matches["Número"]:
            dmc_excluir.add(dmc_num)

    df_f = df_raw[
        df_raw["Estado en pago"].isin(VALID_STATES) &
        ~df_raw["Número"].isin(dmc_excluir)
    ].copy()
...
def main():
    ...
    with col_h1:
        ...
        fecha_max = data["fact"]["Fecha de la factura"].max()
