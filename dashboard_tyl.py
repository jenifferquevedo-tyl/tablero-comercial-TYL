...
def resumen_accesos_hoy(log):
    ...
    personas = [SHORT.get(a, a) for a in ASESORES] + ["Yo", "Gerencia", "Externo 1", "Externo 2"]
    rows = []
    for p in personas:
...
def main():
    ...
    # Identificación de quién abre el tablero -- se pide ANTES de registrar
    # la apertura, para que el registro de gerencia quede asociado a una
    # persona real en vez de anotar "Sin identificar" antes de darle la
    # oportunidad de elegir. No es una autenticación real, ver nota en
    # registrar_visita().
    with st.sidebar:
        st.markdown("#### 👤 Identifícate")
        opciones_id = ["Selecciona tu nombre..."] + [SHORT[a] for a in ASESORES] + ["Yo", "Gerencia", "Externo 1", "Externo 2"]
        ...
        if quien_sel == "Selecciona tu nombre...":
            st.caption("👆 Elige tu nombre para que quede tu ingreso registrado.")
        st.divider()

    # Solo se registra la apertura cuando la persona SÍ elige quién es --
    # así el log no se llena de "Sin identificar" por defecto. El panel de
    # abajo siempre muestra el acumulado real leído de disco.
    if quien_sel != "Selecciona tu nombre...":
